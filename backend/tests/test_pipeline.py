...
class TestTextLayerRung:
    def test_a_digital_pdf_never_reaches_the_ocr_engine(self):
        _, stats, ocr, source = run(
            [digital("RESOLUCION N° 0412/2024"), digital(""), digital("RESOLUCION N° 0555/2024")]
        )
        assert ocr.calls == 0
    ...
    def test_it_reads_the_code_straight_off_the_text_layer(self):
        classifications, _, _, _ = run([digital("RESOLUCION N° 0412/2024")])
        assert classifications[0].code.value == "0412/2024"
...
class TestOcrRungs:
    def test_a_scan_is_read_from_the_header_band_alone(self):
        classifications, _, ocr, source = run(
            [FakePage(header="RESOLUCION N° 0412/2024", body=BODY)]
        )
        assert classifications[0].code.value == "0412/2024"
    ...
    def test_the_full_page_is_only_rendered_when_the_band_comes_back_empty(self):
        classifications, _, ocr, source = run(
            [FakePage(header="", body=f"Se dispone lo siguiente. RESOLUCION N° 0412/2024. {BODY}")]
        )
        assert classifications[0].code.value == "0412/2024"
...
class TestEscalationPolicy:
    def test_a_continuation_page_with_healthy_text_costs_nothing(self):
        ...
        classifications, stats, _, _ = run(
            [digital("RESOLUCION N° 0412/2024"), digital("")], vision=vision
        )
    ...
    def test_a_short_digital_page_is_not_mistaken_for_a_failed_read(self):
        ...
        classifications, stats, _, _ = run(
            [digital("RESOLUCION N° 0412/2024"), FakePage(text=short)], vision=vision
        )
        assert stats.escalated == 0
        assert vision.batches == []
        assert classifications[1].code is None
    ...
    def test_a_short_ocr_page_is_still_escalated(self):
        ...
        vision = FakeVision(answers={2: "0412/2024"})
        _, stats, _, _ = run(
            [digital("RESOLUCION N° 0412/2024"), FakePage(header="", body="ruido ilegible")],
            vision=vision,
        )
    ...
    def test_a_page_with_competing_codes_is_escalated(self):
        ...
        assert stats.escalated == 1
        assert classifications[0].code.value == "0555/2024"
...
class TestBatching:
    def test_escalated_pages_travel_in_mosaics_not_one_request_per_page(self):
        vision = FakeVision()
        pages = [FakePage(header="RESOLUCION N° 0412/2024", body=BODY)]
...
class TestParallelAnalysis:
    def test_fanning_out_preserves_page_order(self):
        pages = [digital(f"RESOLUCION N° {n:04d}/2024") for n in range(1, 33)]
        ...
        assert [c.code.value for c in classifications] == [f"{n:04d}/2024" for n in range(1, 33)]
