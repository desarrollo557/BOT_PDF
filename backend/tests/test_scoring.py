...
HEADER_THEN_CITATIONS = (
    "RESOLUCION No. 00412\n"
    "\n"
    "VISTO la Resolucion No. 00077 y sus modificatorias, y\n"
    "CONSIDERANDO que por Resolucion No. 00099 se aprobo el reglamento"
)
...
class TestDisambiguation:
    def test_the_header_wins_over_cited_resolutions(self):
        ...
        assert selection.code == ResolutionCode.parse("00412")

    def test_a_citation_alone_still_loses_to_nothing_and_is_reported_weak(self):
        text = "VISTO la Resolucion No. 00077 que aprobo el plan anual"
        selection = select_best(extract_candidates(text))
        assert selection.code == ResolutionCode.parse("00077")
...
class TestNeighbourTiebreak:
    def test_sequential_context_breaks_a_tie_between_equal_candidates(self):
        ...
        hinted = select_best(extract_candidates(text), previous_code=ResolutionCode.parse("00555"))
        assert blind.code == ResolutionCode.parse("00412")
        assert hinted.code == ResolutionCode.parse("00555")

    def test_the_hint_never_overrides_a_confident_header(self):
        selection = select_best(
            extract_candidates(HEADER_THEN_CITATIONS),
            previous_code=ResolutionCode.parse("00077"),
        )
        assert selection.code == ResolutionCode.parse("00412")
